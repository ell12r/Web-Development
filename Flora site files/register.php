...
if (!isset($_POST['username'], $_POST['password'], $_POST['email'])) {
	...
	exit('<h1>Please complete the registration form!</h1>');
}
...
if (!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
	exit('<h1>Email is not valid!</h1>');
}
if (preg_match('/^[a-zA-Z0-9]+$/', $_POST['username']) == 0) {
    exit('<b><h1>Username is not valid!</h1></b>');
}
if (strlen($_POST['password']) > 20 || strlen($_POST['password']) < 5) {
	exit('<h1>Password must be between 5 and 20 characters long!</h1>');
}
...
mail($_POST['email'], $subject, $message, $headers);
	...
	echo '<h1>Could not prepare statement!</h1>';
}
	}
	$stmt->close();
} else {
